...
import com.example.junior.First.TabFirstActivity;
...
public class TabHostActivity extends TabActivity implements View.OnClickListener {
    ...
    private LinearLayout ll_first,ll_second,ll_third,ll_forth;
    ...
    private String FORTH_TAG="forth";   //第四个标签的标识串
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        ll_forth=findViewById(R.id.ll_forth);   //获取第四个标签的线性布局
        ...
        ll_forth.setOnClickListener(this);  //给第四个标签注册点击监听器
        ...
        //往标签栏添加第四个标签，其中内容视图展示TabForthActivity
        tab_host.addTab(getNewTab(FORTH_TAG,R.string.menu_first,
                R.drawable.tab_forth_selector, TabForthActivity.class));
        changeContainerView(ll_first);  //默认显示第一个标签的内容视图
    }

    //根据定制参数获得新的标签规格
    private TabHost.TabSpec getNewTab(String spec,int lable,int icon,Class<?> cls){
        //创建一个意图，并存入指定包裹
        Intent intent=new Intent(this,cls).putExtras(mBundle);
        //生成并返回新的标签规格（包括内容意图、标签文字和标签图标）
        return tab_host.newTabSpec(spec).setContent(intent).setIndicator(getString(lable),
                getResources().getDrawable(icon));
    }

    public void onClick(View v){
        if(v.getId()==R.id.ll_first||v.getId()==R.id.ll_second||v.getId()==R.id.ll_third||
                v.getId()==R.id.ll_forth){
    ...
    private void changeContainerView(View v){
        ...
        ll_forth.setSelected(false);    //取消选中第四个标签
        ...
        if(v==ll_first){
            ...
            tab_host.setCurrentTabByTag(FORTH_TAG);    //设置当前标签为第四个标签
        }
    }
}
